...
router.get("/?search", (req, res) => {
  const ownerQuery =
    req.query.ownerInclusion === "is"
      ? req.query.owner
        ? { owner: req.query.owner }
        : {}
      : req.query.owner
      ? { owner: { $nin: req.query.owner } }
      : {};

  const creatorQuery =
    req.query.creatorInclusion === "is"
      ? req.query.creator
        ? { creator: req.query.creator }
        : {}
      : req.query.creator
      ? { creator: { $nin: req.query.creator } }
      : {};

  const subscribedQuery =
    req.query.subscribedInclusion === "all"
      ? req.query.subscribed
        ? { subscribed: { $all: req.query.subscribed } }
        : {}
      : req.query.subscribedInclusion === "any"
      ? req.query.subscribed
        ? { subscribed: { $in: req.query.subscribed } }
        : {}
      : req.query.subscribed
      ? { subscribed: { $nin: req.query.subscribed } }
      : {};

  const tagsQuery =
    req.query.tagsInclusion === "all"
      ? req.query.tags
        ? { tags: { $all: req.query.tags } }
        : {}
      : req.query.tagsInclusion === "any"
      ? req.query.tags
        ? { tags: { $in: req.query.tags } }
        : {}
      : req.query.tags
      ? { tags: { $nin: req.query.tags } }
      : {};
  ...
  const filteredQuery = Object.assign(
    {},
    {
      ...ownerQuery,
      ...creatorQuery,
  ...
  Ticket.find(filteredQuery)
    ...
    .catch((err) =>
      res
        ...
        .json({
          noticketsfound:
            "Sorry, no tickets match your search. Please try again.",
        })
    );
});
