...
class Tags extends React.Component {
  ...
  renderAdded() {
    return Object.values(this.state.added).map((tag) => (
      <div key={tag._id} className="added-item">
        {tag.name}
        <span className="remove" onClick={() => this.remove(tag._id)}>
          {" "}
          x
        </span>
      </div>
  ...
  handleClick(e) {
    ...
    if (clicked) {
      createTag({ name: newTag, orgHandle: currentUser.orgHandle }).then(
        (action) => {
          if (action.tag) {
  ...
  render() {
    ...
    return (
      <div id="tags-container" className="subscribed-container">
        <span className="added">{this.renderAdded()}</span>
        <TagSuggest
        ...
        <div className="newTag">
          {this.state.clicked ? (
            <button className="btn1 add" onClick={(e) => this.handleClick(e)}>
              Create
            </button>
          ) : null}
        </div>
