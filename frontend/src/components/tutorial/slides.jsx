...
class Slides extends React.Component {
    constructor(props) {
        ...
        this.state = { slideCount: localStorage.tutorial === 'true' ? 0 : this.slides.length + 1 };
    }
    ...
    render(){
        ...
        if (slideCount >= slides.length) return (
            <div className="play-tutorial" onClick={e => {
                localStorage.tutorial = true;
                this.handleClick(-1 * slideCount);
            }}>?</div>
        )

        if (localStorage.tutorial === "true") return(
            <div className="slides-and-modal-container">
                <div className="slides-modal" onClick={e => this.handleClick(slides.length + 1)}></div>
                <div className="slide-and-button">
                    <div className="slide-header">
                        <div className="slide-title">
                            Welcome To Tickets
                        </div>
                        <div className="close-tutorial" onClick={e => this.handleClick(slides.length + 1)}>Close</div>
                    </div>
                    ...
                    <div className="slide-buttons">
                        ...
                        <button className="btn1"
                            onClick={e => this.handleClick(1)}
                        >{!slideCount ? 'GET STARTED' : 'CONTINUE'}</button>
                    </div>
                    </div>
                </div>
            </div>
